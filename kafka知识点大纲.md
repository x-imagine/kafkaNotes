...
③max.partition.fetch.bytes（默认1MB）
...
④session.timeout.ms（默认3秒）
    如果consumer在这段时间内没有发送心跳信息，则它会被认为挂掉了。
...
②如果某个消费者处理了几个消息没有提交就死掉了，并提交的偏移量大于消费者正在处理的消息的偏移量，kafka再分区再均衡后，另一个消费者从未提交的地方开始拉取消息。这样就会出现部分消息没有被处理过就被忽略。 
